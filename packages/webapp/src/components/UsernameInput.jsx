...
export default function UsernameInput({
  ...
  useEffect(() => {
    ...
    if (normalized.length < 3) {
      setAvailability({ available: false, reason: 'too_short', message: 'Username must be at least 3 characters' })
      return
    }

    if (normalized.length > 30) {
      setAvailability({ available: false, reason: 'too_long', message: 'Username must be 30 characters or less' })
      return
    }

    if (!/^[a-z][a-z0-9_]*$/.test(normalized)) {
      if (!/^[a-z]/.test(normalized)) {
        setAvailability({ available: false, reason: 'invalid_format', message: 'Username must start with a letter' })
      } else {
        setAvailability({ available: false, reason: 'invalid_format', message: 'Only lowercase letters, numbers, and underscores allowed' })
      }
      return
    }
