...
import type { PublicCardsGridProps } from '../types';

export default function PublicCardsGrid({
  cards = [],
  loading = false,
}: PublicCardsGridProps): JSX.Element {
  if (loading) {
    return (
  ...
  if (cards.length === 0) {
    return <div className="text-center py-8 text-gray-500">No public cards yet</div>;
  }
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {cards.map((card) => (
        ...
        >
          ...
          {/* Answer Preview */}
          <div className="text-xs text-gray-500 line-clamp-2">{card.answer}</div>
