...
import type { FolderListProps, FolderWithColor, SortableFolderTabProps } from '../types';

// Sortable wrapper for folder tabs
function SortableFolderTab({ id, children }: SortableFolderTabProps): JSX.Element {
...
export default function FolderList({
  ...
  const getCardCount = (folderId: string | null): number => {
  ...
  const handleStartEdit = (folder: FolderWithColor): void => {
    setEditingId(folder.id);
    setEditName(folder.name);
  };

  const handleSaveEdit = async (folderId: string): Promise<void> => {
    if (editName.trim()) {
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>, folderId: string): void => {
    if (e.key === 'Enter') {
  ...
  const renderFolderTab = (
    ...
    if (!isUnfiled && folder && editingId === folder.id) {
      return (
        <input
          type="text"
          value={editName}
    ...
    if (!isUnfiled && folder && confirmDeleteId === folder.id) {
      return (
      ...
      <div className="flex gap-2 items-center overflow-x-auto max-md:pb-2 max-md:-mx-1 max-md:px-1 scrollbar-hide">
        <SortableContext items={sortableIds} strategy={horizontalListSortingStrategy}>
          {sortableIds.map((id) => {
            ...
            const folder = isUnfiled ? null : folders.find((f) => f.id === id) ?? null;
