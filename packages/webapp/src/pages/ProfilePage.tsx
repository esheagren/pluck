...
import type { ProfilePageProps, StatusMessage } from '../types';
...
export default function ProfilePage({ user }: ProfilePageProps): JSX.Element {
  ...
  // Initialize edit state from profile - need to handle snake_case to camelCase
  useEffect(() => {
    if (profile) {
      setEditUsername(profile.username || '');
      setEditDisplayName(profile.display_name || '');
      setEditBio(profile.bio || '');
      setEditProfileIsPublic(profile.is_public !== false);
    }
  }, [profile]);
  ...
  const handleProfileSave = async (): Promise<void> => {
    ...
    if (editProfileIsPublic !== (profile?.is_public !== false)) {
      updates.is_public = editProfileIsPublic;
    }
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              <UserAvatar
                ...
                displayName={editDisplayName || profile?.display_name}
              ...
              {profile?.username && (
                <p
                  className={`text-gray-500 dark:text-gray-400 ${
                    profile.display_name ? 'text-sm' : 'text-base font-medium'
                  }`}
                >
                  @{profile.username}
                </p>
          ...
          <div className="space-y-5">
            <div>
              ...
              <ActivityGrid
                ...
                metric="reviews"
                showLegend={false}
                startDate={earliestActivityDate || undefined}
              />
            </div>
            <div>
